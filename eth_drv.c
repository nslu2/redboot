...
//struct eth_drv_sc *__local_enet_sc0 = &npe_netdev0;
//struct eth_drv_sc *__local_enet_sc1 = &npe_netdev1;
...
*/
//snowel:change to IOB 
#define MAX_PORTS 2
CBQ Lan_CBQ[MAX_PORTS];
...
void
eth_drv_write(struct eth_drv_sc *sc, char *buf, int len)
{
...
//
extern struct eth_drv_sc npe_sc0;
extern struct eth_drv_sc npe_sc1;
...
eth_drv_recv(struct eth_drv_sc *sc, int total_len)
{
    ...
    psIOB = LAN_DriverGetIOB(MAX_ETH_MSG);

    if (psIOB) {
      buf = psIOB->dbData;
    } else {
      diag_printf("Can't get new IOB in eth_drv_recv\n");
...
#if 0 //def CYGDBG_IO_ETH_DRIVERS_DEBUG
    if (cyg_io_eth_net_debug) {
        int old_console;
	//snowel
	//        old_console = start_console();
    ...
    if (psIOB) {
        psIOB->dwLength = total_len;
	//        eth_drv_msg_put(&eth_msg_full, msg);

	if (sc == &npe_sc0)
	  putCBToQ(&Lan_CBQ[0], (CB *)psIOB, TAIL);
	else
	  if (sc == &npe_sc1)
	    putCBToQ(&Lan_CBQ[1], (CB *)psIOB, TAIL);
...
//
int
eth_drv_int_vector(struct eth_drv_sc *sc)
{

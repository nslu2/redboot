 ...
 * Handle ICMP packets.
 */
static void default_icmp_handler(IOB *psIOB, ip_route_t *dest);
 ...
 */
void
__icmp_handler(IOB *psIOB, ip_route_t *dest)
{
    (*icmp_handler)(psIOB, dest);
 ...
 */
static void
default_icmp_handler(IOB *psIOB, ip_route_t *dest)
{
    word cksum;
    icmp_header_t *icmp_hdr = IOBDATA(psIOB);

    if (icmp_hdr->type == ICMP_TYPE_ECHOREQUEST
	&& icmp_hdr->code == 0
	&& __sum((word *)icmp_hdr, IOBLEN(psIOB), 0) == 0) {

	icmp_hdr->type = ICMP_TYPE_ECHOREPLY;
	icmp_hdr->checksum = 0;
        cksum = __sum((word *)icmp_hdr, IOBLEN(psIOB), 0);
	icmp_hdr->checksum = htons(cksum);

	IOBOFF(psIOB) -= IP_HDR_SIZE;
	IOBLEN(psIOB) += IP_HDR_SIZE;
        __ip_send(psIOB, IP_PROTO_ICMP, dest);
    }
}

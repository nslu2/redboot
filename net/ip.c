...
#endif
ip_addr_t __local_tftp_server_ip_addr = { 192,168,0,88 };
 ...
 */
void
__ip_handler(IOB *psIOB, enet_addr_t *src_enet_addr)
{
    ip_header_t *ip = IOBDATA(psIOB);
    ip_route_t  r;
    int         hdr_bytes;
    ...
    IOBOFF(psIOB) += hdr_bytes;
    IOBLEN(psIOB) -= hdr_bytes;

    switch (ip->protocol) {

    case IP_PROTO_ICMP:
      __icmp_handler(psIOB, &r);
      break;

#if NET_SUPPORT_TCP
      case IP_PROTO_TCP:
	__tcp_handler(psIOB, &r);
	break;
#endif

    case IP_PROTO_UDP:
      __udp_handler(psIOB, &r);
      break;
 ...
 */
int
__ip_send(IOB *psIOB, int protocol, ip_route_t *dest)
{
    ip_header_t *ip = IOBDATA(psIOB);
    int         hdr_bytes;
    ...
    /* 
     * Dont know why memcpy() cannot work !! Is there any body can tell me?? 
     *                                  Argon Cheng :-(
     */
    memcpy(ip->destination, dest->ip_addr, sizeof(ip_addr_t));

    cksum = __sum((word *)ip, IP_HDR_SIZE, 0);
    ...
    memcpy(&eth_hdr->destination,&dest->enet_addr,sizeof(enet_addr_t));
    net_GetAddr((char *)&eth_hdr->source);
    eth_hdr->type = ntohs(ETH_TYPE_IP);

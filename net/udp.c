 ...
 */
void
__udp_handler(IOB *psIOB, ip_route_t *r)
{
    udp_header_t *udp = IOBDATA(psIOB);
    ip_header_t  *ip = IOBDATA(psIOB) - IP_HDR_SIZE;
    udp_socket_t *s;
	   ...
	   word dest_port, word src_port)
{
  IOB *psIOB;
    ...
    if((psIOB = LAN_DriverGetIOB(ETH_MAX_PKTLEN))==NULL)
      {
	diag_printf("can't get IOB in __udp_send\n");
	return -1;
      }
    IOBOFF(psIOB) = 4 + ETH_HDR_SIZE + IP_HDR_SIZE;

    udp = (udp_header_t *)(IOBDATA(psIOB));
    ip = (ip_header_t *)(IOBDATA(psIOB)- IP_HDR_SIZE);

    IOBLEN(psIOB) = len + sizeof(udp_header_t);
    
    udp->src_port = htons(src_port);
    udp->dest_port = htons(dest_port);
    udp->length = htons(IOBLEN(psIOB));
    udp->checksum = 0;

    memcpy(((char *)udp) + sizeof(udp_header_t), buf, len);

    /* fill in some pseudo-header fields */
    memcpy((char *)ip->source, __local_ip_addr, sizeof(ip_addr_t));
    memcpy((char *)ip->destination, (char *)dest_ip->ip_addr, sizeof(ip_addr_t));
    ip->protocol = IP_PROTO_UDP;
    ip->length = udp->length;

    cksum = __sum((word *)udp, IOBLEN(psIOB), __pseudo_sum(ip));
    udp->checksum = htons(cksum);

    //shift to IP header offset
    IOBOFF(psIOB) -= IP_HDR_SIZE;
    IOBLEN(psIOB) += IP_HDR_SIZE;
    ret = __ip_send(psIOB, IP_PROTO_UDP, dest_ip);
    return ret;
}
               ...
               struct sockaddr_in *local, struct timeval *timo)
{
    ...
    start = SYSTEMTIME_MS;
    res = -1;
    while ((SYSTEMTIME_MS - start) < total_ms) {

...
void do_ping(int argc, char *argv[]);
...
static void
handle_icmp(IOB *psIOB, ip_route_t *src_route)
{
    icmp_header_t *icmp = IOBDATA(psIOB);
    unsigned short cksum;

    if (icmp->type == ICMP_TYPE_ECHOREQUEST
	&& icmp->code == 0
	&& __sum((word *)icmp, IOBLEN(psIOB), 0) == 0) {

	icmp->type = ICMP_TYPE_ECHOREPLY;
	icmp->checksum = 0;
        cksum = __sum((word *)icmp, IOBLEN(psIOB), 0);
	icmp->checksum = htons(cksum);
        __ip_send(psIOB, IP_PROTO_ICMP, src_route);
...
#if 0
    diag_printf("dest_ip's ip is <%u.%u.%u.%u> mac is <%02X:%02X:%02X:%02X:%02X:%02X>\n",
        dest_ip.ip_addr[0], dest_ip.ip_addr[1], dest_ip.ip_addr[2], dest_ip.ip_addr[3],
        dest_ip.enet_addr[0],dest_ip.enet_addr[1],dest_ip.enet_addr[2],
        dest_ip.enet_addr[3],dest_ip.enet_addr[4],dest_ip.enet_addr[5]);
#endif
...
#define DEFAULT_RATE     1000
	...
	if((psIOB = LAN_DriverGetIOB(ETH_MAX_PKTLEN))==NULL) {
	  diag_printf("can't get IOB in %s\n", __FUNCTION__);
	  break;
	}
	IOBOFF(psIOB) = ETH_HDR_SIZE  ;
        ip = IOBDATA(psIOB);
        icmp = IOBDATA(psIOB) + IP_HDR_SIZE;
        IOBLEN(psIOB) = length + sizeof(icmp_header_t);
	...
	icmp->checksum = htons(cksum);
        ...
        ip->length = htons(IOBLEN(psIOB));

        __ip_send(psIOB, IP_PROTO_ICMP, &dest_ip);

        //__pktbuf_free(pkt);

        start_time = SYSTEMTIME_MS;
        timer = start_time + timeout;
        icmp_received = false;
        while (!icmp_received && (SYSTEMTIME_MS < timer)) {
            __enet_poll();
        } 
        end_time = SYSTEMTIME_MS;

        timer = SYSTEMTIME_MS + rate;
        while (SYSTEMTIME_MS < timer) {
            __enet_poll();
        } 

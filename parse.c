          ...
          void **def_arg, int def_arg_type, char *def_descr)
{
    ...
    //    diag_printf("in %s:%d def_arg is <%x> def_arg_type is <%d> argc is <%d> first is <%d>\n", __FILE__, __LINE__, def_arg, def_arg_type, argc, first);
    ...
    for (i = first;  i <= argc;  i++) {
      //    diag_printf("in %s:%d argv[%d] is <%s>\n", __FILE__, __LINE__, i, argv[i]);
        if (argv[i][0] == '-') {
            c = argv[i][1];
            flag_ok = false;
            opt = opts;

...
void do_exec(unsigned long ramdisk,unsigned long kernel);
...
{
    unsigned long ramdisk, kernel;

    if(!argv[2]){
        /* Only one argu, assume kernel address. */
        if(!parse_num(argv[1], &kernel, 0, 0)){
            diag_printf("*** Error: invalid address '%s' for kernel \n", argv[1]);
            return  -1;
        }
    }

    if(!parse_num(argv[1], &ramdisk, 0, 0)){
        diag_printf("*** Error: invalid address '%s' for ramdisk \n", argv[1]);
        return  -1;
    }

    if(!parse_num(argv[2], &kernel, 0, 0)){
        diag_printf("*** Error: invalid address '%s' for kernel \n", argv[1]);
        return  -1;
    }
    
    do_exec(ramdisk, kernel);
}

void 
//do_exec(int argc, char *argv[])
do_exec(unsigned long ramdisk,unsigned long kernel)
{
    unsigned long entry;
    ...
    diag_printf("before go to linux kernel\n");
#ifdef CYGPKG_IO_ETH_DRIVERS
    //Lan_Stop1();
    //    Lan_Stop2();
    //    eth_drv_stop();
